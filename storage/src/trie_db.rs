...
use crate::types::rocksdb::ConfigRocksDB;
...
impl RocksTrieDB {
    pub fn new<P: AsRef<Path>>(
        ...
        if !path.as_ref().is_dir() {
            fs::create_dir_all(&path).map_err(StorageError::CreateDB)?;
        }

        let opts = rocksdb_opts(config)?;
        let db = Arc::new(DB::open(&opts, path).map_err(StorageError::RocksDB)?);
    ...
    fn inner_get(&self, key: &[u8]) -> Result<Option<Vec<u8>>, StorageError> {
        use trie::DB;
...
fn rocksdb_opts(config: ConfigRocksDB) -> Result<Options, StorageError> {
