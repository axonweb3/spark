...
use axon_types::{
    basic::Byte32,
    delegate::{DelegateAtCellData, DelegateCellData},
};
use ckb_types::{
    bytes::Bytes,
...
use common::types::{
    ...
    smt::{Delegator as SmtDelegator, Staker as SmtStaker},
};
...
use crate::ckb::utils::{
    cell_collector::{collect_cells, get_delegate_cell, get_withdraw_cell},
    cell_dep::{
    ...
    script::{always_success_lock, delegate_lock, delegate_smt_type, omni_eth_lock, xudt_type},
    tx::balance_tx,
};
    ...
    for DelegateSmtTxBuilder<C, D>
{
    ...
    async fn build_tx(&self) -> Result<(TransactionView, NonTopDelegators)> {
        ...
        let delegate_type =
            delegate_smt_type(&self.ckb.network_type, &self.type_ids.metadata_type_id);

        let delegate_smt_cells = collect_cells(&self.ckb.client, 1, SearchKey {
            script:               delegate_lock.clone().into(),
            script_type:          ScriptType::Lock,
            filter:               Some(SearchKeyFilter {
                script: Some(delegate_type.clone().into()),
                ..Default::default()
            }),
        ...
        if delegate_smt_cells.len() != 1 {
            return Err(CkbTxErr::SmtCellNum(delegate_smt_cells.len()).into());
        }

        let mut inputs = vec![
            // delegate smt cell
            CellInput::new_builder()
                .previous_output(delegate_smt_cells[0].out_point.clone().into())
                .build(),
        ];
...
struct Statistics {
    ...
    pub invalid_delegate_items: HashMap<Delegator, Vec<DelegateItem>>,
}
...
impl<C: CkbRpc, D: DelegateSmtStorage + Send + Sync> DelegateSmtTxBuilder<C, D> {
    async fn fill_tx(
        ...
        let withdraw_lock = always_success_lock(&self.ckb.network_type);
        let delegate_lock = always_success_lock(&self.ckb.network_type); // todo: no one can delete delegate cell
        ...
        for (delegator, total_amount) in statistics.total_amounts.iter() {
            inputs.push(
                CellInput::new_builder()
                    .previous_output(cells[delegator].out_point.clone().into())
                    .build(),
            );

            let invalid_delegates = statistics.invalid_delegate_items.get(delegator).unwrap();
                ...
                if statistics.withdraw_amounts.contains_key(delegator) {
                    ...
                    (
                        token_cell_data(
                            ...
                            delegate_cell_data(invalid_delegates).as_bytes(),
                        ),
    ...
    async fn collect(&self) -> Result<(Bytes, HashMap<Delegator, Cell>, Statistics)> {
        let (mut cells, mut total_amounts, mut invalid_delegate_items, mut delegates) =
        ...
        for (staker, delegators) in delegates.iter() {
            ...
            let old_smt = self
                .delegate_smt_storage
                .get_sub_leaves(self.current_epoch + 2, smt_staker)
                .await?;
            ...
            self.delegate_smt_storage
                .insert(self.current_epoch + 2, new_delegators)
                .await?;
            ...
            for (staker, in_smt) in staker_info.iter() {
                if !in_smt {
                    let delegate_item = delegates
                        ...
                        .unwrap()
                        .to_owned();
                    invalid_delegate_items
                        .entry(delegator.clone())
                        .and_modify(|e| e.push(delegate_item));
        ...
        for delegator in invalid_delegate_items.clone().keys() {
            if !delegator_delegates.contains_key(delegator) {
                invalid_delegate_items.remove(delegator);
        ...
        Ok((
            delegate_smt_cell_data(roots).as_bytes(),
            cells,
            Statistics {
                non_top_delegators,
    ...
    fn collect_cell_delegates(
        ...
        HashMap<Delegator, Cell>,
        HashMap<Delegator, Amount>,
        HashMap<Delegator, Vec<DelegateItem>>,
        HashMap<TxStaker, HashMap<Delegator, DelegateItem>>,
    )> {
        let mut cells = HashMap::new();
        let mut total_amounts = HashMap::new();
        let mut expired_delegates: HashMap<ckb_types::H160, Vec<DelegateItem>> = HashMap::new();
        let mut delegates: HashMap<TxStaker, HashMap<Delegator, DelegateItem>> = HashMap::new();

        for cell in self.delegate_cells.clone().into_iter() {
            let mut cell_bytes = cell.output_data.clone().unwrap().into_bytes();
            let delegator = Delegator::from_slice(&cell.output.lock.args.as_bytes()[32..]).unwrap();
            ...
            if is_valid {
                cells.insert(delegator.clone(), cell);
                total_amounts.insert(delegator.clone(), total_amount);
            } else {
                // ignore the delegate info if all are expired
                expired_delegates.remove(&delegator);
            }
        }

        Ok((cells, total_amounts, expired_delegates, delegates))
    }
    ...
    async fn remove_non_top_delegators(
        ...
        cells: &mut HashMap<Delegator, Cell>,
    ) -> Result<()> {
        let xudt = xudt_type(&self.ckb.network_type, &self.type_ids.xudt_owner.pack());
        let delegate_cell_lock = omni_eth_lock(&self.ckb.network_type, &staker);

        let delegate_cell = get_delegate_cell(&self.ckb.client, delegate_cell_lock, xudt.clone())
            .await?
            .unwrap();

        let delegate_cell_bytes = delegate_cell.output_data.unwrap().into_bytes();
